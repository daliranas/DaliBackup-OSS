...
const handleCompleteRestore = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  const { taskId } = req.params;
...
hypervisorRouter.post('/agent/complete-restore/:taskId', requireAgentAuth, handleCompleteRestore);
hypervisorRouter.post('/agent/restore-complete/:taskId', requireAgentAuth, handleCompleteRestore);
