...
// In-memory cache for SSL certificates to avoid repeated disk I/O and DB queries
let cachedCertificates: SslCredentials | null = null;
...
export function getOrCreateSslCertificates(forceRegenerate = false): SslCredentials {
  if (!forceRegenerate && cachedCertificates) {
  ...
  if (settings?.ssl_mode === 'CUSTOM' && settings.ssl_cert && settings.ssl_key) {
    ...
    cachedCertificates = { cert: settings.ssl_cert, key: settings.ssl_key };
    return cachedCertificates;
  }
